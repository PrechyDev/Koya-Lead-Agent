...
| GET | ~~`/runs/{id}/export.json`~~ removed (D-100; the CSV is the sample pack) | member | Sample pack. **Drafts are included only for `approved` leads** (outreach-safety approval rule) |
...
| `max_budget_usd` (Claude, per run) | **sized from the run** (D-97): 1 lead $0.72, 3 → $1.43, 5 → $2.14, 10 → $3.63 | $0.72 (1 lead) | SDK option + our ledger check |
...
- **App-level hard stop:** the project budget, $6.00 (`CLAUDE_BUDGET_TOTAL_USD` is the starting value; since D-94 a developer changes it on the Spend page, stored append-only in `lead_agent.budget_changes`; it is a lifetime total, not monthly). Before starting any run, eval or grounding call, the code sums `spend_ledger`. If `spent + this job's cap > total`, it refuses with a clear message. Every Claude call writes to the ledger.
...
| **D-103** | **Discovery doesn't give up on a quiet provider; everyone opens on Qualified.** Up to 30 searches a run (DEV 10); the 20-candidate limit is unchanged (empty searches use none of it). An empty result is retried after 20 s and 60 s instead of immediately (identical input, D-76). After an empty search the tool tells the orchestrator to broaden (1-2 keywords, a synonym, a nearby niche) and never repeat it; the orchestrator keeps searching until the target, the candidate limit or the search limit. ICP queries are 1-3 words. The Leads tab opens on Qualified (then possible fits) for developers too | Confirmed (owner, 2026-09-25: "my last search return no result… we can afford to do 30 runs until we find the number of leads we need"; "the page… starts on the all tab instead of the qualified tab") | Runs 30237325 and c228a963 got 0 companies from every search; the same query "project management software" returned 5 at 20:33 (run 74d6c473) and 0 three times at 20:44: the provider's "Found 0" bursts (errors #63/#65), which immediate retries can't ride out. 3 searches were too few to wait one out | Switching actors (a vendor change); raising the 20-candidate limit (grader constraint) |
| **D-102** | **No agency/consultancy pre-screen.** The free rule from D-98 (3) is removed: agencies and consultancies are Koya's primary customers (PRD business context, D-66), so no company is rejected for being one. What remains is target-relative: the Software Development industry filter only for software objectives, and the Haiku triage, which judges each company against the objective and can drop one only by quoting its own words | Confirmed (owner, 2026-09-25: "we don't need to prescreen for agencies and consultancy. these people are our primary customers") | A word-match rule couldn't tell "an agency, when you asked for SaaS" from "an agency, which is who we sell to"; the triage can, because it reads the objective | Keeping the pre-screen with the "objective asks for services firms" switch (D-98) |
| **D-101** | **Your run follows you**: on every page except the run itself, `#run-notice` polls `/me/run-notice` every 15 s while your run is going ("Your search is running · View"), then shows a dismissible banner once it finishes ("Your search finished: 4 qualified leads · View results"); opening the run marks it seen; polling stops (HTMX 286) when nothing of yours is running | Confirmed (owner, 2026-09-25: "is there a way to be notified that my run is done… like a banner… I can cancel or click to see the runs") | Runs take minutes and people move on to other pages | Browser desktop notifications (permission prompts); email |
| **D-100** | **Stopped runs can be continued; a person settles "needs review"; size bands that overlap pass; clients see qualified first; the CSV is the sample pack.** *Paused* (new status, migration 0011): a restart, crash or shutdown pauses a run; the live panel says "This search stopped before it finished…" and offers **Continue: research N remaining companies / write drafts for N leads / search for more** (`lib/resume.continue_plan`), or **Retry** for a failed run; `/runs/{id}/continue` keeps what's left of the cap or sizes a new one for the remaining work (`continue_cap`), refuses in plain words if the balance can't cover it (no numbers for clients), and restarts the research phase with a "carry on" brief (`orchestrator_continue_prompt`; nothing is researched twice). *Needs review*: the lead panel's **Your call** section (Qualify / Not a fit + note), recorded as human_decision / by / at / note; after Qualify, **Write drafts for this lead** (= Continue). *Headcount*: an "unknown" size passes when LinkedIn's band overlaps the range, with the note in the evidence (`settle_headcount`). *Client view*: the Leads tab opens on Qualified (then possible fits); chips "Qualified · Possible fits to check · All companies checked"; no qualified leads → a callout with **Refine the objective and search again** (also on the live panel). *Export*: one **CSV = the outreach sample pack** per qualified lead (objective, company, website, LinkedIn, fit score, qualification reasoning with evidence, exclusions checked, fit reasons, concerns, sources, who decided, review status/by/at/note, 3 emails, LinkedIn message); drafts only once approved (rule 12), otherwise "not approved yet"; the JSON export is removed | Confirmed (owner, 2026-09-25: pause/resume "click to continue (if it was still drafting, drafting)"; needs review = "Person decides"; headcount = "Band overlaps = pass"; "they just need to see qualified and if nothing qualifies let them refine and search again"; "the exported csv has no drafts… we don't need json… add the linkedin to it and the review status and all of that"; continue budget = what clients need to see: nothing about money) | Runs stopped by restarts lost their work to "failed"; most "needs review" were LinkedIn size bands that couldn't match "20-80"; clients were shown every rejected company; the CSV lacked the drafts the PRD sample pack asks for | Auto-resuming on boot (a laptop would resume Render's runs); showing budget numbers to clients; keeping JSON |
| **D-99** | **Orphaned-run recovery only fails runs that went silent** (5 minutes without the owning server's 1-minute heartbeat), on boot and before a new run starts; and the live panel stops looking busy: a refreshing panel no longer puts its buttons in the loading state ("Cancelling…" flashed every 3 s), and a closed banner stays closed for that run and status | Confirmed (owner, 2026-09-25: "this cancelling appears a lot while the system is running…"; "the banner… I cleared it and it came back") | The database is shared by the laptop and Render: a local server (and my scratchpad audit server) starting up marked Render's live run c7cfea10 "failed" mid-run; it still completed 5/5, but its row kept "Interrupted by server restart" (cleaned). HTMX marks the polling element `htmx-request` on every refresh, and the CSS showed loading text for any button inside it | Keeping boot-time recovery of every active run (unsafe with a shared DB) |
| **D-98** | **Discovery finds companies that can qualify: cheap filters first, expensive research last.** (1) Search terms must name what companies sell/do/serve; bare category words ("B2B SaaS", "SaaS operations") are refused by `save_icp` (`lib/triage.generic_queries`) and the ICP skill turns a vague objective into 3 niches, recorded in assumptions. (2) A software target searches LinkedIn's "Software Development" industry (`industryIds: ["4"]`, from the actor's input schema). (3) ~~A free pre-screen rejected obvious services firms~~ **removed by D-102** (agencies and consultancies are primary customers). (4) One Haiku call (`services/triage.py`, ≤ $0.02, ledger source `triage`, migration 0010) ranks the rest likely/unclear/unlikely; "unlikely" is accepted only with an exact quote of the company's words (checked in code), everything else is researched, likely first; a failed call skips nobody | Confirmed (owner, 2026-09-25: "how else can we do this to be better… it being vague means I should have gotten 10 not 7 and I even got none"; chose all four) | Live run 2553f20d: the vague objective became the searches "B2B SaaS", "SaaS operations", "Software-as-a-service company"; LinkedIn matched names ("SaaS Bro", "B2B Rizz", "Software Allies"); 13 of 15 companies failed "is a SaaS company" after ~$0.13 of Opus research each | Researching everything (the old way); more candidates per run (the 20-candidate rule stays) |
| **D-97** | **Each run's Claude cap is sized from the run**: (companies it may research × $0.13 + leads × $0.05 + $0.05) × 1.15, where companies = min(candidate limit, 2 × leads + 2) (`config.run_cap_usd`; unit costs measured 2026-09-25, in config.py). Start checks reserve a 1-lead run (the lead count is unknown before the ICP step); `save_icp` then sizes the cap for the real target and, if the balance can't cover it, runs the most leads it can pay for, with a note on the ICP tab | Confirmed (owner, 2026-09-25: "why not make the budget for run a bit more generous giving it room to give the user the returns they need?"; chose "scale with run size") | The fixed $0.30 test cap paid for ~2 Opus-researched companies (run 3d638a04: $0.2994, 0 qualified), and $3.00 was tight for 10 leads. A cap is a ceiling, not a charge: a bigger cap only reserves more of the prepaid balance while the run is going | Raising DEV to $0.60 only; keeping $0.30 / $3.00 |
| **D-96** | **The Cancel button survives the 3-second status refresh** (`id="cancel-run"` + `hx-preserve`): HTMX keeps the form as it is instead of redrawing it, so hover, focus and "Cancelling…" aren't lost; when the run ends, the refresh no longer contains it and it goes away | Confirmed (owner, 2026-09-25: "The cancel run button keeps refreshing every few seconds while a run is on.. why?") | The live panel swaps its whole innerHTML every 3 s and the button was inside it | Moving the controls out of the polled panel (more template churn) |
| **D-95** | **Prepaid budget with a monthly (UTC) statement, and a UI pass.** *Budget:* a developer **adds** an amount (not a new total); the balance = starting value + all top-ups − all spend; nothing expires, so each month (00:00 UTC on the 1st) opens with what the last left unspent. Spend gets a **By month** tab (opening, added, spent, carried over; admins too), the developer card shows balance left / spent this month / totals, and "Budget changes" became **Top-ups** (+amount, total after, where it came from). Same append-only table (a top-up = a row with new = old + amount), no migration. *UI:* a header theme switch (Auto follows the device / Light / Dark, stored per browser; `static/theme.js` in `<head>` so there's no flash); "+ New run" first in the nav (and no longer duplicated on Runs); Team invite on one row; one **Manage ▾** menu per person instead of a row of wrapping buttons (fixed-position panel so the table's scroll box can't clip it; Esc / outside click closes); System issues split into issue / what happened + fix / collapsible technical detail / "N times since…" / resolved by; every table: real chevron column, right-aligned numbers, dates and names on one line, objectives clamped to 2 lines with the full text on hover, tool-call time in seconds, checkbox text beside its box | Confirmed (owner, 2026-09-25: "that button becomes for adding to budget and the budget is now monthly (UTC)… the new budget is what ever was left unspent"; chose prepaid; "resets at UTC 00:00"; "the new run should come first… why is deactivate under… can role be on the same column… dark and light mode… look at all the tables") | With full roll-over and top-ups as the only income, a monthly reset can't change the limit, so it's a prepaid balance with monthly reporting, which also keeps rule 5's "$6 total". Dark mode existed but only followed the OS, so it was never visible on a dark-mode laptop. Five buttons per team row wrapped; a menu scales to any number of actions | Monthly allowance + roll-over; monthly without roll-over; icon-only theme switch |
| **D-94** | **The Claude budget lives in the database and a developer changes it in the app; admins see runs left and can request more.** `lead_agent.budget_changes` (migration 0009, append-only: who, when, old → new, reason, and a DB-enforced "Anthropic has credit" confirmation); the current budget is the newest row, else `CLAUDE_BUDGET_TOTAL_USD`. Every guard (`assert_run_fits`, the fact-check guard, the 80% warning, `claude_credit.py`) reads `db.claude_budget()`. Spend page: admins see "About N runs left" (the guard's own arithmetic: remaining ÷ (run cap + $0.10), rounded down) and **Request more budget**, which raises a `budget_requested` System issue (so the developer is emailed) and shows "Requested" until a change resolves it. Developers see the dollars, the change form (new total ≥ spent, ≤ $1000, reason ≥ 5 chars, credit confirmed) and a paginated **Budget changes** tab. Lifetime, not monthly | Confirmed (owner, 2026-09-25: "why can't we just have the dev change it from their end and just ask them if the account has been topped off"; chose in-app developer change, runs left + request button, lifetime for now) | Changing an env var on Render restarts the service (a redeploy that stops a run in progress), for something that happens every time money is added. A normal API key can't read the Anthropic balance, so "has it been topped up" must be a human confirmation, recorded with the change. Admins think in runs, not dollars. The $6 total of rule 5 stays the starting value; raising it is a recorded developer decision | Env var only (restart per change); monthly budget (deviates from the graded "$6 total"; later); letting admins raise it |
| **D-93** | **"+ New run" in the header on every page; the form is never locked by another run; exports only when there are leads; Spend says who sets the budget.** A run already in progress is reported when Start is pressed (the existing 409 banner with a link to it), so the objective typed so far is kept. CSV and sample-pack JSON buttons are disabled with "Nothing to export: this run has no leads" when a run has none. Spend: admins read "The budget is set by your developer"; developers see `CLAUDE_BUDGET_TOTAL_USD` and that "used" includes development and tests | Confirmed (owner, 2026-09-25: "add a button for new run in the nav bar… if something is already running when I click after entering everything, it tells me"; "why is the export… showing… if nothing was returned") | Starting a run shouldn't need a trip to the Runs page; locking the form made people wait before they could even write the objective. An empty export is a dead end. The JSON is the PRD's "outreach sample pack" (objective, ICP, evidence, sources, approved drafts); the CSV is for spreadsheets/CRMs | Keeping the up-front lock; hiding the export buttons (disabled + reason follows design.md) |
