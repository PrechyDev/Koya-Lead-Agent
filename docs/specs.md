...
Env vars: `MODEL_ORCHESTRATOR`, `MODEL_ICP`, `MODEL_RESEARCHER`, `MODEL_COPYWRITER`, `MODEL_GROUNDING`, `MODEL_CHECKS` (scope check + pre-run credit probe, D-75). Chosen by the A/B test (D-77): researcher + copywriter `claude-opus-5-5`, ICP + grounding + checks `claude-haiku-4-5`, orchestrator `claude-sonnet-5`. The final choice and its evidence go in the progress.md A/B table and §13.
