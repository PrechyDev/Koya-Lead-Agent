...
Env vars: `MODEL_ORCHESTRATOR`, `MODEL_ICP`, `MODEL_RESEARCHER`, `MODEL_COPYWRITER`, `MODEL_GROUNDING`, `MODEL_CHECKS` (scope check + pre-run credit probe, D-75). Default before the A/B test: `claude-sonnet-5` everywhere, except grounding and checks = `claude-haiku-4-5`. The final choice and its evidence go in the progress.md A/B table and §13.
...
| E-07 | Apify returns 0 results | Retried up to 2× with the identical input when the run succeeded empty (D-76); if still empty, the first such search is given back (D-54) and the orchestrator tries the next query, within the limits; else `completed_partial` | live cc84694e + test_an_empty_search_is_retried_with_the_same_input, test_a_search_that_stays_empty_stops_after_two_retries |
...
| **D-77** | **Models from the A/B test (proposed, owner to confirm):** ICP **Haiku 4.5**, researcher **Sonnet 5**, fact-check **Haiku 4.5**, copywriter **Sonnet 5** (copy stage not yet measured: Anthropic credit ran out), orchestrator Sonnet 5, checks Haiku 4.5 | Proposed (2026-09-25) | ICP: both passed 3/3 twice, Haiku ~1/3 the cost. Qualification: Haiku marked 4 non-fits "qualified" in 16 answers (the one error the guide forbids), Sonnet 0 and 8/8 on its full repeat. Fact-check: Haiku caught every planted fake, 1 false alarm within the bar, ~1/5 of Sonnet's cost. Full table: progress.md §8 | Sonnet everywhere (3× the ICP cost for no measured gain); Haiku as researcher (false qualifieds) |
