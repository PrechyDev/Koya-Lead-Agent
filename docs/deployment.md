...
3. `DEV_LIMITS=false` from the first deploy (full limits: up to 10 leads, 20 companies, Claude cap sized from the run, $3.63 for 10 leads; D-27/D-33, D-97). The owner sets any other value in Render → Environment if they want smaller runs.
...
2. **Final run on Render** with the PRD objective, full limits (Claude cap $3.63 for 10 leads, sized per run, D-97; + $0.25 Apify per search). It uses most of the remaining $6 budget: if graders must run full runs afterwards, top up Anthropic and raise the budget on the Spend page (developer, D-94); no redeploy needed. Approve drafts in the
