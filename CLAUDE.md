...
4. **Limits are enforced by tools, from the run record.** First search 1.5× the lead target (max 12), top-ups ≤ 5, hard total 20 candidates, up to 30 searches (empty ones use no candidates; D-103), $0.25 Apify cap per actor run, scrape cap, turns, and a Claude cap sized from each run ($0.72 for 1 lead … $3.63 for 10; specs D-97). The agent's requested counts are ignored.
