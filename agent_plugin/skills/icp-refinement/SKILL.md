...
7. **`discovery_query_plan`:** 3 short LinkedIn company-search keyword queries, most specific first. LinkedIn matches them against company **names and descriptions**, so each query must name **what the companies sell, do or who they serve** (a product category, niche or customer type), never just the category: bare "SaaS", "B2B SaaS", "software company" or "SaaS operations" return agencies whose names contain those words, and the server refuses them. Good: `["clinic scheduling software", "field service management software", "property management software"]`. For a **vague objective** ("SaaS companies that might need automation"), choose 3 concrete niches that fit it and the Koya context (small teams with repetitive operations work) and record the choice in `assumptions` ("Searched three niches: …"). Keywords only; geography, size and (for software targets) LinkedIn's Software Development industry are applied as filters automatically.
