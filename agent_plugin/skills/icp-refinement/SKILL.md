...
3. **Not a Koya buyer** (e.g. "ice cream stores in Ife", "barbershops near me", individuals or households): Koya places AI automation assistants with small, growing businesses that have repetitive operations work. Set `is_searchable: false`, `request_type: "too_vague"`, and ask ONE question suggesting suitable targets. Agencies, consultancies, clinics, SaaS, logistics, e-commerce and professional services are all good targets.
   Also: a searchable ICP needs **at least 3 hard filters** after Koya's defaults (e.g. company type, location, size); with fewer, the server asks the person for more detail.
4. **Too vague to search**: the one thing an objective must contain is **what kind of company** (a company type or industry). Without it (e.g. "find me leads", "companies", gibberish), the server turns the run into a clarification question anyway: set `is_searchable: false` and write ONE short, concrete `clarification_question` that offers options, e.g. "Which kind of companies should I look for (for example: US B2B SaaS with 10–100 staff, or UK marketing agencies)?" Do not invent an ICP.
...
7. **`discovery_query_plan`:** 3 short LinkedIn company-search keyword queries, most specific first. LinkedIn matches them against company **names and descriptions**, and a long phrase often matches nothing, so keep each query to **1-3 words**. Each must name **what the companies sell, do or who they serve** (a product category, niche or customer type), never just the category: bare "SaaS", "B2B SaaS", "software company" or "SaaS operations" return agencies whose names contain those words, and the server refuses them. Good: `["clinic scheduling software", "field service software", "automation agency"]`; too narrow: "workflow automation consulting services". For a **vague objective** ("SaaS companies that might need automation"), choose 3 concrete niches that fit it and the Koya context (small teams with repetitive operations work) and record the choice in `assumptions` ("Searched three niches: …"). Keywords only; geography, size and (for software targets) LinkedIn's Software Development industry are applied as filters automatically.
