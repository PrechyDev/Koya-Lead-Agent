...
4. **Limits (checked by code):** subject ≤ 60 characters; body ≤ 120 words; LinkedIn message ≤ 300 characters; **no email addresses, phone numbers or URLs** anywhere in the copy.
...
8. **After `save_outreach`:** if it returns problems (format or unsupported claims), rewrite only what it flags and call it again. You get at most the number of rewrites the tool states; after that, stop.
